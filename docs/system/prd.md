...
| `FR-1376` | **An active *incapacidad* triggers an instruction, not merely a warning.** Where a worker with an active *incapacidad* presents themselves, the capture application records the check-in (`FR-1364`) and then instructs the supervisor to have the worker **leave the installations and check out immediately**. This is the only conflict class where NEO directs an operational action rather than reporting a discrepancy, because a medical certificate says this person should not be working and the exposure compounds every hour they remain — including the possibility of a fresh injury on site while formally incapacitated (consequences subject to `OQ-001`). |
| `FR-1377` | **The prompt check-out is the evidence that matters.** A check-in with no check-out for hours reads as a worker who worked the day. A check-in followed by a check-out minutes later, with a recorded cause, reads as a *patrón* who identified the problem and acted. The elapsed time between the two is therefore surfaced on the *lista de asistencia*, in the *incidencias* report and in the STPS export, alongside the *desviación* documenting what happened (`FR-1330`). Neither event is ever suppressed, edited or netted out: the honest record of an error promptly corrected is a stronger position than a record that shows nothing happened. |
| `FR-1378` | **The failure case is the one that must escalate.** Where the check-out does not follow within a configured interval — the worker stays, or leaves without checking out — the day stops being a documented correction and becomes an unexplained shift worked under an active *incapacidad*. `FR-844` escalates immediately, and the *desviación* remains open and escalating until it is closed with a reason (`FR-1338`). The application continues to prompt for the check-out on the device, because an unprompted supervisor in the middle of a shift change will not remember. |
