...
6. **NEO staff never write to an evidentiary record**, break-glass included — a break-glass
   session runs under a database role with no write privilege on any evidentiary table
   (`FR-1462`). The audit entries recording what a staff member *read* are generated by the
   platform, not authored by them, and are not an exception to this. (`FR-1205`, `INV-066`)
